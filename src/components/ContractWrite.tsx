...
const ContractWrite = () => {
  ...
  const tokenName = "USDC"; // Changed to const
  ...
  switch (chain?.id) {
    ...
    case 80084: // Berachain Bartio Testnet
      contractAddress = "0xd6D83aF58a19Cd14eF3CF6fe848C9A4d21e5727c";
      break;
    case 137: // Polygon
    ...
    case 1513: // Story Protocol Testnet
      contractAddress = "0x91f6F05B08c16769d3c85867548615d270C42fC7";
      break;
    case 5000: // Mantle
  ...
  const convertToUSDCAmount = (amountStr: string) => {
  ...
  if (!chain) {
    return (
      <Wrapper title={`${tokenName} Transfer`}>
        <p>Loading...</p>
      </Wrapper>
    );
  }
  ...
  return (
    <Wrapper title={`${tokenName} Transfer`}>
      <div className="rounded bg-blue-400 px-2 py-1 text-sm text-white">
        Transfer {tokenName} tokens on {chain.name}
      </div>
      ...
      {isError && <p className="text-red-500">Error sending transaction.</p>}
      {address && (
        <Button
          disabled={
            isPending || !receiverAddress || !amount || parseFloat(amount) <= 0
          }
          onClick_={() =>
            writeContract?.({
              abi: ABI,
              ...
              args: [
                ...
                convertToUSDCAmount(amount),
              ],
            })
          }
          cta={`Transfer ${amount} ${tokenName}`}
        />
      )}
    </Wrapper>
  );
